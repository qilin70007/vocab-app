...
for (const dir of tinaContents) {
  const candidate = 'E:\\Tina\\' + dir;
  try {
    const stat = fs.statSync(candidate);
    if (stat.isDirectory()) {
      // Check if data dir exists inside
      const dataCandidate = candidate + '\\data';
      if (fs.existsSync(dataCandidate)) {
        // Check for words.json
        const wordsPath = dataCandidate + '\\words.json';
        if (fs.existsSync(wordsPath)) {
          BASE = candidate;
          dataDir = dataCandidate;
...
const originalPath = dataDir + '\\words.json';
const ocrPath = BASE + '\\ocr_parsed_words_v2.json';
const mergedPath = BASE + '\\merged_words.json';
