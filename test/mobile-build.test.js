...
test('mobile entry declares offline mode before the application script starts', () => {
  ...
  assert.match(mobileHtml, /手机离线版 v2\.5\.8/);
...
test('APK build verifies the packaged offline entry and uses a visible app version', () => {
  ...
  assert.match(workflow, /vocab-app-v2\.5\.8-debug\.apk/);
  assert.match(nativePatch, /VocabMasterAndroid\/2\.5\.8/);
  assert.match(nativePatch, /versionCode 20508/);
  assert.match(nativePatch, /versionName "2\.5\.8"/);
});
